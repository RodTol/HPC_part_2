...
void plot_data_2d(char *name, int n1, int n2, int n3, int n1_local,
  ...
  if (dir == 1) {
    i1 = n1 / 2 - 1; // Selecting the slice in the middle

    if (mype == owner) {
      fp = fopen(buf, "w");

      for (i2 = 0; i2 < n2; ++i2) { // Print on file along the first dimension
        for (i3 = 0; i3 < n3; ++i3) {
          index = index_f(i1 - n1_local_offset, i2, i3, n1_local, n2, n3);
      ...
      fclose(fp);
             ...
             dir == 3) { // In these 2 cases the slices are scattered
    i2 = n2 / 2 - 1;
    i3 = n3 / 2 - 1;
    int nk = (dir == 2) ? n3 : n2;
    int ik;
    ...
    buffer = (double *)malloc(n1 * nk * sizeof(double)); // Contains the slice along the direction
    ...
    if (dir == 2) { // Select one direction
      for (i3 = 0; i3 < n3; ++i3) {
        ...
        MPI_Gatherv(local_buffer, n1_local, MPI_DOUBLE, buffer1d, sizes, displ,
                    MPI_DOUBLE, 0,
                    MPI_COMM_WORLD); // Gathering each vertical colum

        for (i1 = 0; i1 < n1; ++i1) {
          buffer[i1 * n3 + i3] = buffer1d[i1]; // Copying each vertical column
        }
      }
    } else { // Or the other
      for (i2 = 0; i2 < n2; ++i2) {
        ...
        for (i1 = 0; i1 < n1; ++i1) {
          buffer[i1 * n2 + i2] = buffer1d[i1]; // Copying each vertical column
        }
      }
    }
    ...
    if (mype == 0) {
      ...
      for (i1 = 0; i1 < n1; ++i1) {
        for (ik = 0; ik < nk; ++ik) {
          fprintf(fp, "%14.6f", buffer[i1 * nk + ik]);
        }
